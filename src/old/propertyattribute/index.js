...
export default class PropertyAttribute{
  constructor(options){
    // super(options)
    if(!options)options={};
    ...
    const npp = new NavigationPanel({panels:[]})
    // const npp = new NavigationPanel({panels:[rsp,rpp,rap]})


    const pill = new Pill({id:"Style",active:true,content:npp})

    this.np =  new NavigationPill({style:'layerproperty',pills:{rendering:pill}})
